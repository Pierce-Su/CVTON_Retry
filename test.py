...
if opt.phase == "test":
    ...
    for i, data_i in enumerate(test_dataloader):
        ...
        pred, parsing, C_transform = model(image, label, "generate", None, agnostic=agnostic)
        ...
        print(f"C_transform type: {type(C_transform)} =====================")
        ...
        if opt.dataset == "mpv":
            ...
            filename = data_i['name'][0].split("/")[-1]
        im_saver.save_images(image['target_cloth'], "fake_target_cloth", i)
        ...
        cv2.imwrite(os.path.join("results", opt.name, opt.phase + "_images", filename[:-4] + "_cloth.png"), cloth)
